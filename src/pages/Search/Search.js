...
function Search() {
    ...
    if (loading) {
        ...
        return (
            <>
                <h2 className="ooms-search-header">Kết quả tìm kiếm <span className='search-value'>{searchValue}</span></h2>
                ...
                <div className='ooms-search-container'>
                    ...
                    <div className="ooms-search-song">
                        {dataSearch.songs.map((item, index) => {
                            return <ItemSong style="circle" props={item} list={dataSearch.songs} key={index}></ItemSong>
                        })}
                    </div>
