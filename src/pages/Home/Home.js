...
function Home() {
    ...
    if (loading) {
        ...
        const dataNew = data.filter(item => item.sectionType === "new-release")[0].items.all.filter((item2, index2) => index2 < 5);

        const dataTrending = data.filter(item => item.sectionType === "RTChart")[0].items
        const dataSinger = data.filter(item => item.sectionType === "artistSpotlight")[0].items.filter((item2, index2) => index2 < 6);

        const dataTop100Title = data.filter(item => item.sectionId === "h100")[0].title
        const dataTop100 = data.filter(item => item.sectionId === "h100")[0].items.filter((item2, index2) => index2 < 3);
                    ...
                    >
                        {dataSlide.map((item, index) => {
                            return <SwiperSlide key={index}>
                                {item.type === 1 ? <img src={item.banner} alt={item.description} onClick={() => context.handleClickSong(item)} /> : <Link to={item.link} state={item.encodeId}><img src={item.banner} alt={item.description} />
                                </Link>}
                    ...
                    <div className="ooms-container-right">
                        ...
                        <div className="ooms-content-song">
                            {dataTrending.map((item, index) => {
                                return <ItemSong style="circle" props={item} list={dataTrending} key={index}></ItemSong>
                            })}
                        </div>
                    </div>
                </div>
